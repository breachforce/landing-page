...
{ 
    ...
    {      
        ...
        this.speed   = 3;//reduced speed for subtler effect
        ...
        this.junctionMemoryLength = 4; //reduced length for cleaner look
    } 
    ...
    { 
        ...
        // Using more subtle colors for background effect
        var colors = 
        [
            `rgba(220,38,38,${opacity})`,//red-600 to match theme
            `rgba(239,68,68,${opacity})`,//red-500
            `rgba(185,28,28,${opacity})`,//red-700
            `rgba(127,29,29,${opacity})`,//red-900
            `rgba(75,85,99,${opacity})`,//gray-600
            `rgba(107,114,128,${opacity})`,//gray-500
            `rgba(55,65,81,${opacity})`,//gray-700
    ...
    */
        ...
        for(let i = 0; i < 3; i++)
        {
            ...
            switch(i)//create three circles with same center
            {
                case 0: 
                    ...
                    color   = this.getColor(0.8,false,colorIndex); 
                    break; 
                case 1: 
                    radius  =circleradius *   2;//bigger circle 
                    color   = this.getColor(0.4,false,colorIndex);             
                    break; 
                case 2: 
                    radius  =circleradius *   4;//biggest circle (reduced from 6)
                    color   = this.getColor(0.1,false,colorIndex); 
                    break; 
            }
            //draw the node
            ctx.beginPath(); 
    ...
    { 
        ...
        if(this.velocity.x === 0)//if gridworm is moving vertically
        {
            ...
            {
                points.push({x:x+this.interval/4,y:y});//point to the right (reduced size)
                points.push({x:x-this.interval/4,y:y});//point to the left 
                points.push({x:x,y:y+this.interval/4});//point below 
            }
            else//if gridworm is moving up
            {
                points.push({x:x+this.interval/4,y:y});//point to the right
                points.push({x:x-this.interval/4,y:y});//point to the left
                points.push({x:x,y:y-this.interval/4});//point above  
            }
        }
        ...
        {
            let point = points[i];
            this.drawCircle(point.x,point.y,circleradius/3,ctx,colorIndex); 
        } 
    ...
    drawTriangle(point1,point2,point3,ctx)
    {
        ...
        ctx.fillStyle = 'rgba(220,38,38,0.1)';//subtle red theme color
        ctx.fill();   
    }
    draw(ctx)
    {    
        ...
        {   
            ...
            //draw painted squares at every junction point with reduced opacity
            ctx.fillStyle   = this.getColor(0.05,false,this.mainColorIndex); 
            ctx.fillRect(junction.point.x,junction.point.y,this.interval,this.interval);
            
        } 
        //draw the line connecting head to body
        ctx.strokeStyle = this.getColor(0.3,false,this.mainColorIndex);
...
class Painter
{
    ...
    {      
        this.screenWidth    = screenWidth;
        this.screenHeight   = screenHeight;   
        this.interval       = 50;//increased interval for less density
    ...
    {
        let gridworms = [],
            numOfGridWorms = 15; //reduced number for cleaner look
...
function getBrowserWindowSize() 
{
    let win = window,
    doc     = document,
    offset  = 0,// removed offset for full coverage
...
document.addEventListener('DOMContentLoaded', initGridWormAnimation);
